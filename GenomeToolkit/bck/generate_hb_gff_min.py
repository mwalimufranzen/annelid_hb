...
def create_target_with_flanking(seq, exon_positions):
    """Create a target sequence with 3 bp flanking regions."""
    min_pos = min([start for _, start, _, _ in exon_positions])
    max_pos = max([end for _, _, end, _ in exon_positions])

    # Add 3 bp flanking sequences
    flanking_start = max(min_pos - 3, 0)
    flanking_end = min(max_pos + 3, len(seq))
    target_seq = seq[flanking_start:flanking_end]

    target_filename = "target_min.fasta"
    with open(target_filename, 'w') as file:
        target_record = SeqRecord(Seq(target_seq),
                                  id="target",
                                  description="Target sequence with 3bp flanking regions")
...
def main():
    ...
    # Create the target sequence with 3 bp flanking regions and adjust exon positions
