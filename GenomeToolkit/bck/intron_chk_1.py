...
def excise_exons_introns_to_fasta(exon_positions, intron_positions, seq):
    ...
    for exon_name, start, end, _ in exon_positions:
        exon_seq = seq[start-1:end]
        with open(f"{exon_name}.fasta", "w") as exon_file:
            exon_file.write(f">{exon_name}\n{exon_seq}\n")

    for intron_name, start, end, _ in intron_positions:
        intron_seq = seq[start-1:end]
        with open(f"{intron_name}.fasta", "w") as intron_file:
            intron_file.write(f">{intron_name}\n{intron_seq}\n")
