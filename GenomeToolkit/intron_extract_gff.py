...
def write_gff_file(gff_file, exon_positions, seqid):
    ...
    with open(gff_file, "w") as gff:
        ...
        sorted_exons = sorted(exon_positions.items(), key=lambda x: x[1][0])

        for exon_name, (start, end) in sorted_exons:
            if start is not None and end is not None:
                # Write exon information
                gff.write(f"{seqid}\tgenbank\t{exon_name}\t{start}\t{end}\t.\t+\t0\tattributes\n")
        
        # Now handle introns based on sorted exons
        for i in range(1, len(sorted_exons)):
            previous_exon_name, (previous_start, previous_end) = sorted_exons[i-1]
            current_exon_name, (current_start, current_end) = sorted_exons[i]

            if previous_end < current_start - 1:
                intron_start = previous_end + 1
                intron_end = current_start - 1
                intron_name = f"intron{i}"  # Name the introns based on their position
...
def write_intron_fasta_files(target_seq, exon_positions):
    ...
    for i in range(1, len(sorted_exons)):
        ...
        if previous_end < current_start - 1:
            ...
            intron_name = f"intron{i}"  # Name the intron based on its order
