...
def compare_sequences_with_alignment(seq1, seq2, output_file="comparison.txt"):
    """Performs pairwise alignment on two sequences and writes the alignment to a file."""
    
    # Run Biopython's PairwiseAligner
    alignment = run_pairwise_alignment(seq1, seq2)
    
    # Write the alignment result to comparison.txt
    with open(output_file, "w") as output:
        output.write("Pairwise Alignment:\n")
        for aln in alignment:
            output.write(str(aln))
            output.write("\n\n")
    
    print(f"Alignment successfully written to {output_file}")
...
# Compare the sequences and write the alignment output to comparison.txt
